...
export class SettingsService {
  ...
  savePreferences(patch: Partial<DesktopPreferences>) {
    if(!object(patch)||Object.keys(patch).some(k=>!['behavior','permission','shortcuts','sessionRenames','sessionAccessModes','autoArchive','archiveRetentionDays','defaultThinkingLevel','openWithApp','memoryAssist'].includes(k)))throw new Error('不支持的设置项');
